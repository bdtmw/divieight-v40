...
const STEPS = ["License", "Insurance/Cert", "FinCEN/Ethics", "Broker Link", "Complete"] as const;

export type AgentOnboardingStep = 1 | 2 | 3 | 4 | 5;

export function AgentOnboardingStepper({ current }: { current: AgentOnboardingStep }) {
  return (
    <ol className="mx-auto flex w-full max-w-3xl items-center gap-1.5 sm:gap-2">
      {STEPS.map((label, i) => {
        const step = (i + 1) as AgentOnboardingStep;
